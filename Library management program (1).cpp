...
{
   public:
       char bookname[100],authorname[50],bookId[20],publication[50];
       int q,B,p;
       Lib()
       {
...
void Lib::show(int i)
{
                        ...
                        if(i==1)
                            {
                                ...
                                while(!intf.eof())
                                    {
                                        for(i=0;b==B&&bookname[i]!='\0'&&st1[i]!='\0'&&(st1[i]==bookname[i]||st1[i]==bookname[i]+23);i++);
  ...
  void Lib::see(int x)
  {
      ...
      if(i==1)
      {
          ...
          while(!intf.eof())
          {
            for(i=0;b==B&&q!=0&&bookname[i]!='\0'&&ch[i]!='\0'&&(ch[i]==bookname[i]||ch[i]==bookname[i]+23);i++);
...
void Lib::issue()
{
    ...
    else if(i==3)
    {
        ...
        while(!intf.eof())
        {
              for(i=0;st1[i]!='\0'&&st1[i]!='\0'&&st1[i]==st1[i];i++);
              if(st1[i]=='\0'&&st1[i]=='\0')
              {
                  cont++;
                  if(cont==1)
    ...
    while(!intf.eof())
        {
            for(i=0;st[i]!='\0'&&st1[i]!='\0'&&st1[i]==st[i];i++);
            for(j=0;st1[j]!='\0'&&st[j]!='\0'&&st[j]==st1[j];j++);
            if(st[i]=='\0'&&st1[j]=='\0'&&st[j]=='\0'&&st1[i]=='\0')
                {
                    d=q;
                    m=B;
                    y=p;
    ...
    while(!intf.eof())
        {
            for(i=0;st[i]!='\0'&&st1[i]!='\0'&&st1[i]==st[i];i++);
            for(j=0;st1[j]!='\0'&&st[j]!='\0'&&st[j]==st1[j];j++);
            if(st[i]=='\0'&&st1[j]=='\0'&&st[j]=='\0'&&st1[i]=='\0'&&cont==0)
                {
                    cont++;
                    intf.read((char*)this,sizeof(*this));
...
void Lib::der(char st[],int b,int x)
{
    ...
    while(!intf.eof())
    {
        for(i=0;b==B&&st[i]!='\0'&&st[i]!='\0'&&st[i]==st[i];i++);
        if(st[i]=='\0'&&st[i]=='\0')
        {
            cont++;
            if(x==1)
